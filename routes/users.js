...
async function login(req, res) {
  ...
  if (!mail || !password)
    return res
      .status(401)
      .send("L'email et le mot de passe sont des champs obligatoires");
  ...
  if (user === null)
    return res
      .status(401)
      .send("L'utilisateur que vous avez entré n'existe pas");
  ...
  if (!user.isActivate)
    return res
      .status(401)
      .send(
        "Veuillez contacter le responsable, votre compte n'est pas encore activé"
      );
...
async function addUser(req, res) {
  ...
  if (!validator.isEmail(user.mail))
    return res.status(401).send("Veuillez entrer un mail valide");

  const existingUser = await User.findOne({ mail: user.mail });
  if (existingUser)
    return res
      .status(401)
      .send("Cet email est déjà utilisé, veuillez choisir un autre");
